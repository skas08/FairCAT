...
# FairCAT load function
def load_faircat(args, dataset, runs=5):
    data_dir = f"data/{dataset}/"
    features = np.load(f"{data_dir}/features.npy")
    labels_np = np.load(f"{data_dir}/labels.npy")
    edges = np.load(f"{data_dir}/edges.npy")
    sens = np.load(f"{data_dir}/sens.npy")
    ...
    train_masks, val_masks, test_masks, idx_trains, idx_vals, idx_tests = make_train_val_test_indexes(
        args, runs, data_dir, sens, labels_np)
...
def get_dataset(dataname, runs, args):
    if(dataname == 'credit'):
        ...
        load = load_yago
    elif dataname == 'pokec_n_faircat':
        load = load_faircat
    elif(dataname == 'german_faircat'):
        load = load_faircat
    elif dataname == 'faircat_scaling_15':
        load = load_faircat
    elif(dataname == 'faircat_scaling_20'):
        load = load_faircat
    elif dataname == 'faircat_scaling_25':
        load = load_faircat
    elif dataname == 'faircat_balancing_balanced':
        load = load_faircat
    elif dataname == 'faircat_balancing_mild_imbalance':
        load = load_faircat
    elif dataname == 'faircat_balancing_strong_imbalance':
        load = load_faircat
    elif dataname == 'faircat_correlations_high':
        load = load_faircat
    elif dataname == 'faircat_correlations_medium':
        load = load_faircat
    elif dataname == 'faircat_correlations_low':
        load = load_faircat
